...
Future<void> _pickAndUploadProfileImage() async {
  ...
  'statusSeenBy': <String, bool>{},
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Could not update profile image: $e'),
                ...
                child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final double bannerScale =
                        (data?['bannerScale'] as num?)?.toDouble() ?? 1.0;

                    final double bannerOffsetX =
                        (data?['bannerOffsetX'] as num?)?.toDouble() ?? 0.0;

                    final double bannerOffsetY =
                        (data?['bannerOffsetY'] as num?)?.toDouble() ?? 0.0;

                    final String profileImageUrl =
                        (data?['profileImageUrl'] ?? '').toString();

                    final String statusText =
                        (data?['statusText'] ?? 'Tap to set your status.')
                            .toString();

                    final bool hasHebrew =
                        RegExp(r'[\u0590-\u05FF]').hasMatch(statusText);
